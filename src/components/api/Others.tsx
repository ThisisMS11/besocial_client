...
export async function fetchOtherProfileInfo(userId: string | undefined) {
    try {
        ...
        if (token) {
            ...
            const response = await axios.get<UserInfo>(`${import.meta.env.VITE_APP_URL_LOCAL}/api/v1/user/other/info/${userId}`, config);
...
export async function fetchOtherProfilePosts(userId: string | undefined) {
    try {
        ...
        const response = await axios.get<PostResponse>(`${import.meta.env.VITE_APP_URL_LOCAL}/api/v1/user/other/posts/${userId}`, config)
